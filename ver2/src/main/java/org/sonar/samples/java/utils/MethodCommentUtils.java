...
//				&& !getType(methodComments.get(firstAnotationIndex - 1)).equals(Type.EMPTY_LINE)) {
...
//		}
	...
	public static void check(JavaFileScannerContext javaFileScannerContext, MethodTree methodTree) {
		try {
			...
			String desErrMsg = commentLineTypes.get(Type.INVALID_DESCRIPTION_FORMAT).isEmpty()?validSection(Type.DESCRIPTION):"";
			...
			String exceptErrMsg = commentLineTypes.get(Type.INVALID_EXCEPTION_FORMAT).isEmpty()?validSection(Type.EXCEPTION):"";
