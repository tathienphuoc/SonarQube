...
public class IfStatementTreeUtils {
	...
	public static void remove(String variableName) {
		for (int i = 0; i < IfConditions.size(); i++) {
			IfConditions.set(i, IfConditions.get(i).replaceAll("[|(&]" + variableName + "[=!]=null", "")
					.replaceAll("[|(&]null[=!]=" + variableName, ""));
		}
		IfConditions = IfConditions.stream().map(el -> el.replaceAll("[|(&]" + variableName + "[=!]=null", "")
				.replaceAll("[|(&]null[=!]=" + variableName, "")).collect(Collectors.toList());
	}
