 ...
 * 
 * @author tathienphuoc
 * @see CheckICWE209Rule_b4AddDebugStatement
 * @since J2EE 1.6
 */
@Rule(key = "CheckICWE209Rule")
public class CheckICWE209Rule_b4AddDebugStatement extends BaseTreeVisitor implements JavaFileScanner {
	...
	public void visitMethodInvocation(MethodInvocationTree tree) {
		try {
			...
			if ((logVars.contains(varName) || (localLogVars.containsKey(varName) && localLogVars.get(varName)))
					&& getMethodInvocationLine(tree).startsWith(varName + ".error")) {
	...
	public void visitCatch(CatchTree tree) {
		try {
			...
			if (hasThrows) {
				...
				System.out.println(String.format(formatErr, CommonMessage.ABSENT_THROW_NEW));
				...
				System.out.println(String.format(formatErr, CommonMessage.ABSENT_LOG_ERROR));
